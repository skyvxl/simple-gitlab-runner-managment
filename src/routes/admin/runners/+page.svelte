<script lang="ts">
  ...
  async function deleteRunner() {
    ...
    try {
      ...
      if (response.ok) {
        allRunners = allRunners.filter((r: any) => r.id !== id);
  ...
  const runnersByOwner = $derived(() => {
    return allRunners.reduce(
      (acc: Record<string, any[]>, runner: any) => {
        ...
        {#each Object.entries(runnersByOwner) as [owner, runners]}
          <section>
            <h2
              ...
              style="margin-bottom: 16px; border-bottom: 1px solid rgb(var(--m3-scheme-outline-variant)); padding-bottom: 8px;"
            >
              Owner: {owner}
            </h2>
...
<Dialog open={showDeleteDialog} headline="Delete Runner" onclose={() => (showDeleteDialog = false)}>
  <div class="m3-body-medium" style="margin-bottom: 24px;">
    Are you sure you want to delete runner <strong>"{runnerToDelete?.name}"</strong> owned by
    <strong>{runnerToDelete?.owner}</strong>?
  </div>
  {#snippet buttons()}
